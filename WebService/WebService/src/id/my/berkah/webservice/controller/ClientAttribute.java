 ...
 * @author dadan
 */
@WebServlet(name = "ClientAttribute", urlPatterns = {"/ClientAttribute"})
public class ClientAttribute extends HttpServlet 
{
            ...
            throws ServletException, IOException, ParseException, GeneralSecurityException 
    {
        ...
        String  userApi = "";
        String  pwdApi = "";
        String  format = "userApi=xxx&pwdApi=xxx&reserveNo=xx&attrCode=xx&attrValue=xx";
                ...
                {   
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (node[0].toUpperCase().equals("ATTRCODE"))
                                {
                                    if (node.length > 1)
                                    {
                                        attrCode = node[1];
                                ...
                                {
                                    if (node[0].toUpperCase().equals("ATTRVALUE"))
                                    {
                                        if (node.length > 1)
                                        {
                                            attrValue = node[1].replace("\n", "").replace("\r", "").replace("null", "");;
                                        }
                                    }
                                    else
                                    {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ClientAttribute.class.getName()).log(Level.SEVERE, null, ex);
    ...
    ResultStatus updateData(String userApi, String pwdApi, String addr, String reserveNo, String attrCode, String attrValue)
    {
        ...
        if ((Integer)ua.get("outError") == 0)
        {
             ...
             {
                Map result = model.ClientAttribute(reserveNo, attrCode, attrValue);
    ...
    private String converStringToUrlEncoded(String data) throws UnsupportedEncodingException 
    {
        String encodedString = java.net.URLEncoder.encode(data, "UTF-8");
